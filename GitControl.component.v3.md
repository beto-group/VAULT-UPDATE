...
// Using your modularized approach for the library
const { diff_match_patch } = await dc.require(dc.headerLink("GitControl.component.v3.md", "diff_match_patch"));
...
function GitControl() {
  // ====================================================================
  //                        -- YOUR SETTINGS --
  const fileName = "GitControl.component.v3.md";
  // ====================================================================
  ...
  // --- NEW STATE to track the absolute latest commit ---
  ...
  // --- NEW GENERIC REVERT/CHECKOUT FUNCTION ---
  // A helper to avoid code duplication between revert and return-to-latest
  const checkoutVersion = async (hash, actionVerb) => {
      try {
        setStatusMessage(`${actionVerb} to version ${hash.slice(0, 7)}...`);
        const file = app.vault.getAbstractFileByPath(fileName);
        if (!file) {
            setStatusMessage(`Error: Source file not found, cannot ${actionVerb}.`);
            return;
        }
        
        const reconstructedContent = await rebuildFileFromHistory(hash);
        await app.vault.modify(file, reconstructedContent);
        
        // The useEffect will automatically run again, creating a new commit.
        setStatusMessage(`Successfully ${actionVerb}d to ${hash.slice(0, 7)}. A new commit has been created.`);
        setSelectedVersion(''); // Reset dropdown after action
    } catch (error) {
        console.error(`${actionVerb} Error:`, error);
        setStatusMessage(`Error during ${actionVerb}: ${error.message}`);
    }
  }

  const handleRevert = async () => {
    if (!selectedVersion || (history.length > 0 && selectedVersion === history[0].hash)) {
      setStatusMessage("Cannot revert: Select a previous version.");
      return;
    }
    await checkoutVersion(selectedVersion, 'revert');
  };
  
  // --- NEW HANDLER for the "Return to Latest" button ---
  const handleReturnToLatest = async () => {
    if (!absoluteLatestHash || (history.length > 0 && absoluteLatestHash === history[0].hash)) {
        return; // Safety check, button shouldn't be visible anyway.
    }
    await checkoutVersion(absoluteLatestHash, 'return');
  }
    ...
    const processFileAndCommitChanges = async () => {
      try {
        ...
        if (await adapter.exists(objectPath)) {
          ...
          // This is a new version, commit it. (Logic is unchanged)
          ...
          let commitObject;
          if (!(await adapter.exists(headFile))) {
            commitObject = { source: fileName, timestamp: new Date().toISOString(), content: fullFileContent };
          } else {
            ...
            commitObject = { source: fileName, timestamp: new Date().toISOString(), parent: previousHash, patch: dmp.patch_toText(patches) };
          }
        ...
        const fetchHistory = async () => {
            const historyList = [];
            if (!(await adapter.exists(headFile))) return;
            
            let currentHashInHistory = await adapter.read(headFile);
            let seenHashes = new Set(); // Prevent infinite loops in case of corrupt history

            while (currentHashInHistory && !seenHashes.has(currentHashInHistory)) {
                seenHashes.add(currentHashInHistory);
                const objectPath = `${objectsDir}/${currentHashInHistory}.json`;
                if (!(await adapter.exists(objectPath))) break;

                const commitObject = JSON.parse(await adapter.read(objectPath));
                historyList.push({
                    hash: currentHashInHistory,
                    timestamp: commitObject.timestamp,
                });
                currentHashInHistory = commitObject.parent || null;
            }
            setHistory(historyList);
            
            // --- NEW LOGIC: Find the absolute latest commit by timestamp ---
            if(historyList.length > 0) {
                const latestCommitByTimestamp = historyList.reduce((latest, current) => {
                    return new Date(current.timestamp) > new Date(latest.timestamp) ? current : latest;
                });
                setAbsoluteLatestHash(latestCommitByTimestamp.hash);
            } else {
                setAbsoluteLatestHash('');
            }
        };
        await fetchHistory();
        ...
        for (const match of allMatches) {
            ...
            if (headerMatch !== undefined) lastHeader = headerMatch.trim();
            else if (codeMatch !== undefined) {
                parsedBlocks.push({ title: lastHeader || `Block ${untitledBlockCounter++}`, content: codeMatch.trim() });
                lastHeader = null;
            }
        }
        
        setCodeBlocks(parsedBlocks.length > 0 ? parsedBlocks : []);
        setActiveTabIndex(0);

        // Update status message at the end
        const headHash = await (adapter.exists(headFile) ? adapter.read(headFile) : '');
        setStatusMessage(`Loaded version ${headHash.slice(0, 7)}`);
  ...
  return (
    <dc.Stack style={{ padding: "10px 15px", fontFamily: "monospace" }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '10px', marginBottom: '10px', background: '#f9f9f9', padding: '10px', borderRadius: '4px', border: '1px solid #eee' }}>
        <label htmlFor="history-select" style={{fontWeight: 'bold'}}>History:</label>
        <select
          id="history-select"
        ...
        >
          <option value="">-- Select a version to revert to --</option>
          {history.map((commit, index) => (
            <option key={commit.hash} value={commit.hash} disabled={index === 0}>
              {index === 0 ? 'Current: ' : ''}
        ...
        <button
          ...
          disabled={!selectedVersion || (history.length > 0 && selectedVersion === history[0].hash)}
          style={{padding: '5px 15px', cursor: !selectedVersion || (history.length > 0 && selectedVersion === history[0].hash) ? 'not-allowed' : 'pointer'}}
        >
          Revert
        </button>
        {/* --- NEW: Conditional "Return to Latest" button --- */}
        {absoluteLatestHash && history.length > 0 && absoluteLatestHash !== history[0].hash && (
            <button
                onClick={handleReturnToLatest}
                title={`Return to the most recent version: ${absoluteLatestHash.slice(0,7)}`}
                style={{padding: '5px 15px', cursor: 'pointer', background: '#dff0d8', border: '1px solid #c3e6cb', color: '#155724'}}
            >
                Return to Latest
            </button>
        )}
