...
const { diff_match_patch } = await dc.require(dc.headerLink("GitControl.component.v4.md", "diff_match_patch"));
...
function GitControl() {
  // ====================================================================
  //                        -- YOUR SETTINGS --
  const fileName = "GitControl.component.v4.md";
  // ====================================================================
  ...
  // --- THIS IS THE KEY FIX ---
  // The checkout operation must update the HEAD pointer itself.
  const checkoutVersion = async (hash) => {
      try {
        ...
        // Explicitly update the HEAD to point to the checked-out version.
        // This was the missing piece.
        await adapter.write(headFile, hash);

        // Trigger the useEffect to re-scan and update the UI state.
  ...
  const handleVersionSelect = async (e) => {
    ...
    if (newHash && newHash !== currentHeadHash) {
        await checkoutVersion(newHash);
    }
  };

  const handleReturnToLatest = async () => {
    if (!absoluteLatestHash || absoluteLatestHash === currentHeadHash) {
        return;
    }
    await checkoutVersion(absoluteLatestHash);
  }
  ...
  useEffect(() => {
    const processFileAndCommitChanges = async () => {
      try {
        ...
        // We only commit if the file's content is *new* and not already a commit object.
        const fullFileContent = await app.vault.read(file);
        const currentContentHash = await calculateHash(fullFileContent);
        const objectPath = `${objectsDir}/${currentContentHash}.json`;
        ...
        if (!(await adapter.exists(objectPath))) {
          ...
          await adapter.write(headFile, currentContentHash);
        }

        // Now, read the state of the system *after* any potential commit.
        ...
        for (const objPath of allObjectFiles) {
            try {
                const commit = JSON.parse(await adapter.read(objPath));
                const hash = objPath.split('/').pop().replace('.json', '');
                if (commit.timestamp) allCommits.push({ hash, timestamp: commit.timestamp });
            } catch (e) { /* ignore */ }
        }
        ...
        // UI updates
        const regex = /(?:^# (.*))|(?:^```([^\r\n]*)\r?\n([\s\S]*?)\r?\n```)/gm;
        const parsedBlocks = [...fullFileContent.matchAll(regex)].reduce((acc, match) => {
            if (match[1] !== undefined) acc.lastHeader = match[1].trim();
            else if (match[3] !== undefined) {
                acc.blocks.push({ title: acc.lastHeader || `Block ${acc.untitled++}`, content: match[3].trim() });
                acc.lastHeader = null;
            }
            return acc;
  ...
  // --- The JSX is now correct and will work with the fixed logic ---
  return (
    <dc.Stack style={{ padding: "10px 15px", fontFamily: "monospace" }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '10px', marginBottom: '10px', background: '#f9f9f9', padding: '10px', borderRadius: '4px', border: '1px solid #eee' }}>
        ...
        <select
          id="history-select"
          value={currentHeadHash}
          onChange={handleVersionSelect}
          disabled={history.length < 1}
          style={{flexGrow: 1, padding: '5px', cursor: 'pointer'}}
        >
          {history.map((commit) => (
            <option key={commit.hash} value={commit.hash}>
              {new Date(commit.timestamp).toLocaleString()} ({commit.hash.slice(0, 7)})
            </option>
          ))}
        </select>
      ...
      <pre style={{ background: '#f4f4f4', border: '1px solid #ddd', padding: '1em', borderRadius: '5px', whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: '#333', margin: "0" }}>
        <code>
          {codeBlocks.length > 0 && codeBlocks[activeTabIndex]
            ...
            : '// No code blocks were found in the file or the file is empty.'}
        </code>
      </pre>
    </dc.Stack>
  );
