...
const { useAceEditor } = await dc.require(dc.headerLink("GitControl.component.v8.md", "UseAceEditor"));
const { useState, useEffect, useRef } = dc;
const { diff_match_patch, DIFF_DELETE, DIFF_INSERT, DIFF_EQUAL } = await dc.require(dc.headerLink("GitControl.component.v8.md", "diff_match_patch"));
...
function GitControl() {
  // --- SETTINGS ---
  const fileName = "GitControl.component.v8.md";
  ...
  if (isCompareMode) {
    ...
    tabsToShow = singleViewBlocks; // Use the dedicated state for single view
  ...
  const rebuildFileFromHistory = async (targetHash) => {
    ...
    // The working example uses patch_fromText. We stick to it for compatibility, ensuring JSON safety.
    if (commitObject.parent && commitObject.patch) {
      const parentContent = await rebuildFileFromHistory(commitObject.parent);
      // Ensure patch is a string before passing to fromText
      const patchText = typeof commitObject.patch === 'object' ? dmp.patch_toText(commitObject.patch) : commitObject.patch;
  ...
  // *** USING THE ROBUST LOGIC FROM THE WORKING v6 EXAMPLE ***
  const checkoutVersion = async (hash) => {
    try {
        setStatusMessage(`Checking out version ${hash.slice(0, 7)}...`);
        const file = app.vault.getAbstractFileByPath(fileName);
        if (!file) {
            setStatusMessage("Error: Source file not found during checkout.");
            return;
        }
  ...
  useEffect(() => {
    const processFileAndCommitChanges = async () => {
      try {
        ...
        // This check is the core of the working logic: does an object for this hash already exist?
        const objectPath = `${objectsDir}/${currentDiskHash}.json`;
        if (!(await adapter.exists(objectPath))) {
          ...
          if (!headExists) {
            ...
            try {
                ...
                const patches = dmp.patch_make(previousContent, sourceFileContent);
                // Use patch_toText for storage, matching the working v6 example's logic.
                commitObject = { source: fileName, timestamp: new Date().toISOString(), parent: previousHash, patch: dmp.patch_toText(patches) };
            } catch (e) {
                console.error("Failed to create patch, creating full commit as fallback.", e);
                commitObject = { source: fileName, timestamp: new Date().toISOString(), content: sourceFileContent };
            }
          }
        ...
        setCurrentHeadHash(headHash);
        
        // The rest of the logic to populate history and UI state remains the same.
        const allCommits = [];
        ...
        setAbsoluteLatestHash(allCommits.length > 0 ? allCommits[0].hash : '');
        ...
        // The single view should always show the content from the file on disk, which is our source of truth.
        setSingleViewBlocks(parseContentIntoBlocks(sourceFileContent));
  ...
  // --- MINIMAP LOGIC (Unchanged, already performant) ---
        ...
        <dc.Stack spacing={3} style={{ padding: "10px 15px", fontFamily: "monospace" }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '10px', background: '#f9f9f9', padding: '10px', borderRadius: '4px', border: '1px solid #eee' }}>
                {isCompareMode ? (
                    <>
                        <label>Compare:</label>
                        <select value={compareHashA} onChange={e => setCompareHashA(e.target.value)} disabled={history.length < 1} style={{flexGrow: 1, padding: '5px', cursor: 'pointer'}} >{history.map((commit) => (<option key={commit.hash} value={commit.hash}>{new Date(commit.timestamp).toLocaleString()} ({commit.hash.slice(0, 7)})</option>))}</select>
                        <label>With:</label>
                        <select value={compareHashB} onChange={e => setCompareHashB(e.target.value)} disabled={history.length < 1} style={{flexGrow: 1, padding: '5px', cursor: 'pointer'}} >{history.map((commit) => (<option key={commit.hash} value={commit.hash}>{new Date(commit.timestamp).toLocaleString()} ({commit.hash.slice(0, 7)})</option>))}</select>
                    </>
                ) : (
                    <>
                        <label htmlFor="history-select" style={{fontWeight: 'bold'}}>Version:</label>
                        <select id="history-select" value={currentHeadHash} onChange={e => checkoutVersion(e.target.value)} disabled={history.length < 1} style={{flexGrow: 1, padding: '5px', cursor: 'pointer'}} >{history.map((commit) => (<option key={commit.hash} value={commit.hash}>{new Date(commit.timestamp).toLocaleString()} ({commit.hash.slice(0, 7)})</option>))}</select>
                        {absoluteLatestHash && absoluteLatestHash !== currentHeadHash && (
                            <button onClick={() => checkoutVersion(absoluteLatestHash)} title={`Return to the most recent version`} className="git-control-button" style={{ background: '#dff0d8', border: '1px solid #c3e6cb', color: '#155724'}}>Go to Latest</button>
                        )}
                    </>
                )}
...
const { loadScript } = await dc.require(dc.headerLink("GitControl.component.v6.md", "LoadScript"));
...
function useAceEditor({
    ...
    useEffect(() => {
        ...
        if (editor && editor.getValue() !== code) {
            ...
            const pos = editor.session.selection.toJSON(); // Save cursor position
            editor.setValue(code, -1);
            editor.session.setMode(`ace/mode/${language || 'javascript'}`);
            editor.session.selection.fromJSON(pos); // Restore cursor position
            editor.focus();
            setTimeout(() => { isProgrammaticChangeRef.current = false; }, 50); // Reset flag after a short delay
        }
    }, [code, language]);
    ...
    // EFFECT 7: Manages minimap creation, the hidden clone editor, and all synchronization (PEAK PERFORMANCE).
    useEffect(() => {
        ...
        return cleanup;
    }, [isMinimapVisible, code]); // Re-run if minimap is toggled or content changes drastically.
