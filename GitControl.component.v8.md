...
function GitControl() {
  ...
  const checkoutVersion = async (hash) => {
    try {
        ...
        // Physically modify the source file on disk to match the selected version.
        // This keeps the file system, HEAD, and view in perfect sync.
        await app.vault.modify(file, reconstructedContent);
        
        // This component is now "up to date" with the checked-out version.
        await app.vault.adapter.write(headFile, hash);
        
        // Trigger a re-render. The main useEffect will now see that the file
        // on disk matches HEAD and will not create a new commit.
  ...
  useEffect(() => {
    const processFileAndCommitChanges = async () => {
      try {
        ...
        // The logic is now simple: if the file on disk is different from HEAD, it's a new commit.
        if (currentDiskHash !== headHash) {
            ...
            if (!(await adapter.exists(newObjectPath))) {
                let commitObject;
                // If HEAD is empty, or we can't rebuild from it, create a new root commit.
                if (!headHash) {
        ...
        // Setup for compare mode default
...
const { loadScript } = await dc.require(dc.headerLink("GitControl.component.v8.md", "LoadScript"));
